...
func getUserFieldValue(u *map[string]interface{}, s string, custom map[string]string) string {
	...
	r := reflect.ValueOf(SQLImportConf.User.AccountMapping)
...
func getProfileFieldValue(u *map[string]interface{}, s string, custom map[string]string) string {
	//-- Dyniamicly Grab Mapped Value
	r := reflect.ValueOf(SQLImportConf.User.ProfileMapping)
...
func loggerGen(t int, s string) string {
	...
	if t < SQLImportConf.Advanced.LogLevel && t != 0 {
		return ""
	}
...
func deletefiles(path string, f os.FileInfo, err error) (e error) {
	...
	cutoff = time.Duration(SQLImportConf.Advanced.LogRetention) * cutoff
	now := time.Now()
...
func runLogRetentionCheck() {
	logger(1, "Processing Old Log Files Current Retention Set to: "+fmt.Sprintf("%d", SQLImportConf.Advanced.LogRetention), true)

	if SQLImportConf.Advanced.LogRetention > 0 {
		//-- Curreny WD
		cwd, _ := os.Getwd()
		//-- Log Folder
		logPath := cwd + "/log"
