...
func getSiteFromLookup(importData *userWorkingDataStruct) string {
	...
	if SQLImportConf.User.Site.Value == "" {
		logger(4, "Site Lookup is Enabled but Attribute is not Defined", false)
		return ""
	}
	//-- Get Value of Attribute
	logger(1, "DB Attribute for Site Lookup: "+SQLImportConf.User.Site.Value, false)

	//-- Get Value of Attribute
	siteAttributeName := processComplexField(importData.DB, SQLImportConf.User.Site.Value)
