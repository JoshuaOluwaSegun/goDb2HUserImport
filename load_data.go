...
func initXMLMC() {
	...
	pageSize = SQLImportConf.Advanced.PageSize
...
func loadUsersRoles() {
	...
	if SQLImportConf.User.Role.Action != "Create" && SQLImportConf.User.Role.Action != "Update" && SQLImportConf.User.Role.Action != "Both" {
...
func loadSites() {
	//-- Only Load if Enabled
	if SQLImportConf.User.Site.Action != "Create" && SQLImportConf.User.Site.Action != "Update" && SQLImportConf.User.Site.Action != "Both" {
...
func loadGroups() {
	boolSkip := true
	for index := range SQLImportConf.User.Org {
		orgAction := SQLImportConf.User.Org[index]
